...
*mentors – club_members [1:М]* обязательная, не идентифицирующая    
...
*heads – club_members [1:М]* обязательная, идентифицирующая    
...
*departments – SMM [1:M]* обязательная, не идентифицирующая    
...
*SMM - rubrics [1:M]* обязательная, не идентифицирующая       
...
*departments – PR [1:M]* обязательная, не идентифицирующая    
...
*departments – SMM [1:M]* обязательная, не идентифицирующая    
В СММ департаменте может существовать несколько медиа каналов. Первичный ключ не мигрирует, поэтому связь не идентифицирующая.    
*departments – Knowledge_base [1:M]* обязательная, не идентифицирующая    
...
*departments – Events [1:M]* обязательная, не идентифицирующая    
В департаменте Events организуют множество мероприятий. Первичный ключ не мигрирует, поэтому связь не идентифицирующая.         
*departments – Projects* [1:M] обязательная, не идентифицирующая     
В департаменте Projects регулируются все проекты, реализуемые внутри клуба . Первичный ключ не мигрирует, поэтому связь не идентифицирующая.      
*club_members – departments [M:M]* обязательная, в дальнейшем распадается.      
...
Azure Data Studio – это бесплатный, кроссплатформенный инструмент с
открытым исходным кодом для работы с базами данных Microsoft SQL Server,
разработанный компанией Microsoft.

<details><summary>Небольшой список того что умеет Azure Data Studio</summary>

    1. Написание, редактирование и выполнение SQL запросов, иными словами,
    это некий редактор SQL кода.
    2. Импорт CSV файлы, и даже подключать целые каталоги со скриптами и с
    данными.
    3. Просмотр определений объектов базы данных
    В Azure Data Studio есть возможность просматривать сведения об
    объектах базы данных, иными словами, в процессе написания SQL
    запросов или инструкций, Вы очень легко можете узнать исходную
    инструкцию создания той или иной таблицы, хранимой процедуры или
    других объектов базы данных.
    Azure Data Studio | Dbeaver 2
    4. Редактирование данных в табличном виде
    Azure Data Studio позволяет редактировать данные в таблицах, используя
    привычный табличный вид. Для этого необходимо в обозревателе
    объектов щелкнуть правой кнопкой мыши по целевой таблице и выбрать
    пункт «Edit Data»
    5. Поиск объектов в базе данных (таблици, колонки, процедуры и т.п.);
    6. Экспорт и Импорт в/из БД/файл в форматре csv xlsx
    7. Расширяемость функционала
    Azure Data Studio позволяет работать не только с Microsoft SQL Server, но и
    с другими СУБД, например, с PostgreSQL, Oracle [нет MySQL]
    Например, чтобы подключиться к PostgreSQL и начать работать с базами
    данных этой СУБД, необходимо установить специальное расширение,
    которое доступно по умолчанию

</details>

<img width="650" alt="image" src="https://user-images.githubusercontent.com/76436391/170858328-95e748c5-fb84-4ce5-a5a5-b503771e6fda.png">



<details><summary>Список поддерживаемых Azure Data Studio баз данныхo</summary>
...
<img width="650" alt="image" src="https://user-images.githubusercontent.com/76436391/170858465-be10e98d-239a-4f3c-8b18-438a770a4997.png">

DBeaver - это свободный кроссплатформенный менеджер баз данных для для работы с различными СУБД. О DBeaver кратко: много возможностей, небольшой вес, портабельность, свободное ПО.

<details><summary>Небольшой список того что умеет DBeaver</summary>
...
<img width="650" alt="image" src="https://user-images.githubusercontent.com/76436391/170858699-4ba71fa6-1cb8-47ee-a91e-d4e0cb116ef7.png">
